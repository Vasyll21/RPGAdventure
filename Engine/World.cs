...
namespace Engine
{
    public static class World
    {
        ...
        private static void PopulateLocations()
        {
            ...
            Location townSquare = new Location(LOCATION_ID_TOWN_SQUARE, "Town square", "You see a fountain. Near fountain stay a person. He's a rat-cather.");

            Vendor nafanjaTheRatCather = new Vendor("Nafanja the Rat-Cather");
            nafanjaTheRatCather.AddItemToInventory(ItemByID(ITEM_ID_PIECE_OF_FUR), 5);
            nafanjaTheRatCather.AddItemToInventory(ItemByID(ITEM_ID_RAT_TAIL), 3);

            townSquare.VendorWorkingHere = nafanjaTheRatCather;

            Location alchemistHut = new Location(LOCATION_ID_ALCHEMIST_HUT, "Alchemist's hut", "There are many strange plants on the shelves.");
            alchemistHut.QuestAvailableHere = QuestByID(QUEST_ID_CLEAR_ALCHEMIST_GARDEN);

            Location alchemistsGarden = new Location(LOCATION_ID_ALCHEMISTS_GARDEN, "Alchemist's garden", "Many plants are growing here.");
            alchemistsGarden.EnemyLivingHere = EnemyByID(MONSTER_ID_RAT);

            Location farmhouse = new Location(LOCATION_ID_FARMHOUSE, "Farmhouse", "There is a small farmhouse, with a farmer in front.");
            farmhouse.QuestAvailableHere = QuestByID(QUEST_ID_CLEAR_FARMERS_FIELD);

            Location farmersField = new Location(LOCATION_ID_FARM_FIELD, "Farmer's field", "You see rows of vegetables growing here.");
            farmersField.EnemyLivingHere = EnemyByID(MONSTER_ID_SNAKE);
            ...
            spiderField.EnemyLivingHere = EnemyByID(MONSTER_ID_GIANT_SPIDER);
